    ...
    @foreach($messages as $message)
      <div class="group flex items-start space-x-3 relative cursor-pointer transition-colors duration-200 hover:outline-none hover:ring-2 hover:ring-indigo-600 focus:outline-none focus:ring-2 focus:ring-indigo-600 p-1 rounded">
        ...
        <div class="absolute right-0 top-0 hidden group-hover:flex space-x-2 p-1 bg-gray-800 rounded">
          <button title="react" class="transform transition-transform duration-300 hover:scale-110">
          ...
          <button title="reply" class="transform transition-transform duration-300 hover:scale-110">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-4">
          ...
          @if ($message->user_id === Auth::id())
            <button title="edit" class="transform transition-transform duration-300 hover:scale-110">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-4">
            ...
            <div x-data="{ open: false }">
              <!-- Trash Button -->
              <button title="trash" @click="open = true" class="transform transition-transform duration-300 hover:scale-110">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-4">
                  <path stroke-linecap="round" stroke-linejoin="round"
             ...
             48.667 0 0 0-7.5 0" />
              ...
              <!-- Modal -->
              <!-- TODO: Use one modal and pass message ID to it -->
              <div x-show="open" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
                <div class="bg-grey-800 p-6 rounded-lg shadow-lg max-w-sm w-full">
                  <h2 class="text-lg font-semibold text-white mb-4">Wanna delete this message?</h2>
                  <div class="flex justify-end space-x-3">
                    <button @click="open = false" class="px-4 py-2 bg-gray-400 rounded hover:bg-gray-300">Cancel</button>
